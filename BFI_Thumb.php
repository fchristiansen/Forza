 ...
 *          'quality' int 1-100
 *          'fit' bool
 *          'fit_color' string hex-color #000000-#ffffff
 *          'force_jpeg' bool
 *	
...
if ( ! class_exists( 'BFI_Image_Editor_Imagick_1_3' ) ) {
	...
	class BFI_Image_Editor_Imagick_1_3 extends WP_Image_Editor_Imagick {
        ...
        /** Resizes/scales image to fit into boundaries.
         *
         * @access public
         *
         * @param int $max_w
         * @param int $max_h
         * @param bool|string hex $fit_color false for transparency or color in hex RGB e.g. #ff00ff or 
         * @return boolean|WP_Error
         */
        public function resize_fit( $max_w, $max_h, $fit_color = false ) {
            $size = $this->get_size();
            if ( ( $size['width'] == $max_w ) && ( $size['height'] == $max_h ) )
                return true;
            $dims = image_resize_dimensions( $size['width'], $size['height'], $max_w, $max_h, $crop );
            if ( ! $dims )
            ...
            try {
                ...
                if ( $fit_color !== false )
                    $this->image->setImageBackgroundColor( $fit_color );
                $w = $this->image->getImageWidth();
                $h = $this->image->getImageHeight();
                $this->image->extentImage( $max_w, $max_h, ( $w - $max_w) / 2, ( $h - $max_h ) / 2 );
            }
            catch ( Exception $e ) {
                return new WP_Error( 'image_resize_fit_error', $e->getMessage() );
            }
...
if ( ! class_exists( 'BFI_Image_Editor_GD_1_3' ) ) {
	...
	class BFI_Image_Editor_GD_1_3 extends WP_Image_Editor_GD {
        ...
        protected function _resize_fit( $width, $height, $fit_color ) {
            $new_image = @imagecreatetruecolor( $width, $height );
            if ( $new_image === false )
                return false;
            if ( $fit_color === false || $this->mime_type == 'image/png' ) {
                imagesavealpha( $new_image, true );
                $background_color = imagecolorallocatealpha($new_image, 0, 0, 0, 127);
                if ( $background_color === false )
                    return false;
                imagefill( $new_image, 0, 0, $background_color );
            }
            $size = $this->get_size();
            imagecopyresampled( $new_image, $this->image, 0, 0, 0, 0, $width, $height, $size['width'], $size['height'] );
            imagedestroy( $this->image );
        ...
        public function resize_fit( $max_w, $max_h, $fit_color = false ) {
            $size = $this->get_size();
            if ( $size['width'] > $max_w ) {
                $ratio = $max_w / $size['width'];
                $height = $size['height'] * $ratio;
                if ( $this->_resize_fit( $max_w, $height, $fit_color ) === false )
                    return new WP_Error( 'image_resize_fit_error', __('Image resize fit failed.', 'default'), $this->file );
            }
            $size = $this->get_size();
            if ( $size['height'] > $max_h ) {
                $ratio = $max_h / $size['height'];
                $width = $size['width'] * $ratio;
                if ( $this->_resize_fit( $width, $max_h, $fit_color ) === false )
                    return new WP_Error( 'image_resize_fit_error', __('Image resize fit failed.', 'default'), $this->file );
            }
            $new_image = @imagecreatetruecolor($max_w, $max_h);
            if ( $new_image === false )
                return new WP_Error( 'image_resize_fit_error', __('Image resize fit failed creating new image.', 'default'), $this->file );
            if ( $fit_color !== false ) {
                $hexColor = preg_replace('#^\##', '', $fit_color);
                $r = hexdec (substr ($hexColor, 0, 2));
                $g = hexdec (substr ($hexColor, 2, 2));
                $b = hexdec (substr ($hexColor, 2, 2));
                $background_color = imagecolorallocate( $new_image, $r, $g, $b );
                if ( $background_color === false )
                    return new WP_Error( 'image_resize_fit_error', __('Image resize fit failed allocating color.', 'default'), $this->file );
                imagefill( $new_image, 0, 0, $background_color );
            } else {
                imagesavealpha($new_image, true);
                $background_color = imagecolorallocatealpha($new_image, 0, 0, 0, 127);
            ...
            $size = $this->get_size();
            $newx = floor( ( $max_w - $size['width'] ) / 2 );
            $newy = floor( ( $max_h - $size['height'] ) / 2 );
            $image_copy_result = @imagecopy( $new_image, $this->image, $newx, $newy, 0, 0, $size['width'], $size['height'] );
            if ( $image_copy_result === false )
                return new WP_Error( 'image_resize_fit_error', __('Image resize fit failed copying to new image.', 'default'), $this->file );
            imagedestroy( $this->image );
            $this->image = $new_image;
            $this->update_size();
            return true;
	...
	class BFI_Thumb_1_3 {
	    ...
	    public static function thumb( $url, $params = array(), $single = true ) {
	        ...
	        $suffix = (string) filemtime( $img_path ) .
                ...
                ( isset( $fit ) ? ( $fit ? '1' : '0' ) : '0' ) .
                ( isset( $fit_color ) ? str_pad( preg_replace( '#^\##', '', $fit_color ), 8, '0', STR_PAD_LEFT ) : '00000000' ) .
                ( isset( $force_jpeg ) ? ( $force_jpeg ? '1' : '0' ) : '0' );
            ...
            // If fit color is transparent, change the image type to png
            if ( ! empty( $fit ) && ( ! isset( $fit_color ) || $fit_color === false ) ) {
                $ext = 'png';
            }
            if ( $force_jpeg ) {
                $ext = 'jpg';
	        ...
	        if ( @file_exists( $destfilename ) && getimagesize( $destfilename ) ) {
	            ...
	            if ( $crop_only === false ) {
	                if ( ( isset( $width ) && $width ) || ( isset( $height ) && $height ) ) {
	                    if ( ! empty( $fit ) && method_exists( $editor, 'resize_fit' ) ) {
                            if ( ! isset ( $fit_color ) ) {
                                $fit_color = false;
                            }
                            if ( is_wp_error( $editor->resize_fit( $width, $height, $fit_color ) ) ) {
                                return false;
                            }
                        } else {
	            ...
	            // save our new image
	            $mime_type = ( isset( $opacity ) || ( ! empty( $fit ) && ( ! isset($fit_color) || $fit_color === false ) ) ) ? 'image/png' : null;
                if ( $force_jpeg ) {
